...
type Step =
  ...
  | { name: 'ollama-detect' }
  | { name: 'openrouter-key'; defaultModel: string }
...
function ProviderChooser({
  ...
  const localMetadata = getProviderPresetUiMetadata('llama-cpp')
  const openrouterMetadata = getProviderPresetUiMetadata('openrouter')
  const openAIMetadata = getProviderPresetUiMetadata('openai')
  const geminiMetadata = getProviderPresetUiMetadata('gemini')
  const mistralMetadata = getProviderPresetUiMetadata('mistral')
  const helperText =
    'Coreling is local-first. Use Coreling Local for private inference, or Coreling Cloud (included — no API key).'
  const options: OptionWithDescription<ProviderChoice>[] = [
    {
      label: localMetadata.label,
      value: 'llama-cpp',
      description: localMetadata.description,
    },
    {
      label: openrouterMetadata.label,
      value: 'openrouter',
      description: openrouterMetadata.description,
    },
    {
      label: 'Auto',
      value: 'auto',
      description:
        'Prefer local llama.cpp when available, otherwise guide setup',
    },
    {
      label: openAIMetadata.name,
      value: 'openai',
      description: 'OpenAI and other OpenAI-compatible APIs',
    },
    {
...
export function ProviderWizard({
  ...
  switch (step.name) {
        ...
        <ProviderChooser
          onChoose={value => {
            if (value === 'auto') {
              ...
              const defaults = getProviderPresetDefaults('llama-cpp')
              finishProfileSave(onDone, 'llama-cpp', {
                OPENAI_BASE_URL: defaults.baseUrl,
                OPENAI_MODEL: defaults.model,
              })
            } else if (value === 'openrouter') {
              const defaults = getProviderPresetDefaults('openrouter')
              if (hasBundledOpenRouterAccess()) {
              ...
              onDone(`Removed saved provider profile at ${filePath}. Restart Coreling to go back to normal startup.`, {
                display: 'system',
              })
            } else {
    ...
    case 'openrouter-key':
      {
        const openrouterMetadata = getProviderPresetUiMetadata('openrouter')
        const bundled = hasBundledOpenRouterAccess()
      return (
        <TextEntryDialog
          resetStateKey={step.name}
          title={`${openrouterMetadata.label} setup`}
          subtitle="Step 1 of 1"
          description={
            bundled
              ? 'Optional: enter your own OpenRouter key, or leave blank to use Coreling Cloud (included).'
              : process.env.OPENROUTER_API_KEY
                ? `Enter an API key, or leave blank to reuse ${openrouterMetadata.credentialEnvVars[0] ?? 'OPENROUTER_API_KEY'} from this session.`
                : 'Enter your OpenRouter API key. Prompts are sent to cloud models — not private.'
          }
          initialValue=""
          placeholder="sk-or-..."
          mask="*"
          allowEmpty={bundled || Boolean(process.env.OPENROUTER_API_KEY)}
          validate={value => {
            const candidate =
              value.trim() ||
              process.env.OPENROUTER_API_KEY ||
              (bundled ? resolveBundledOpenRouterApiKey() : '')
            return sanitizeApiKey(candidate)
              ? null
              : 'Enter a real API key, or use Coreling Cloud with a blank key.'
          }}
          onSubmit={value => {

...
import {
  buildCorelingModelOptions,
  findPresetByModel,
  isCorelingSectionHeader,
} from '../../constants/corelingModels.js'
import {
  applyCorelingModelByName,
  corelingModelSwitchHint,
} from '../../utils/model/corelingModelSwitch.js'
...
async function loadModelDiscoveryContext(): Promise<ModelDiscoveryContext | null> {
  if (BRAND_NAME.startsWith('Coreling')) {
    return loadCorelingModelDiscoveryContext()
  }
...
function loadCorelingModelDiscoveryContext(): ModelDiscoveryContext {
  return {
    kind: 'coreling',
    autoRefresh: false,
    canRefresh: false,
    discoveryState: {
      message:
        'Spark · Chat · Pro (local) — downloads if missing. Free cloud models via OpenRouter.',
      tone: 'info',
    },
    optionsOverride: buildCorelingModelOptions() as ModelOption[],
...
function ModelPickerWrapper({
  ...
  const [switchingLabel, setSwitchingLabel] = React.useState<string | null>(
    null,
  )
  ...
  const handleSelect = (model: string | null, effort: EffortLevel | undefined) => {
    if (model && isCorelingSectionHeader(model)) {
      return
    }
    ...
    if (discoveryContext?.kind === 'coreling' && model) {
      ...
      void applyCorelingModelByName(model).then(switchResult => {
        ...
        if (switchResult.error) {
          onDone(switchResult.error, { display: 'system' })
          return
        }
        finishModelSelect(
          model,
    ...
    finishModelSelect(model, effort, '')
  }

  function finishModelSelect(
    model: string | null,
  ...
  if (switchingLabel) {
    return <LoadingState label={switchingLabel} />
  }
  ...
  return (
    <ModelPicker
      ...
      headerText={
        discoveryContext?.kind === 'coreling'
          ? 'Spark · Chat · Pro  —  free cloud models'
          : undefined
      }
...
function SetModelAndClose({
  ...
  React.useEffect(() => {
    async function handleModelChange(): Promise<void> {
      ...
      const corelingPreset = findPresetByModel(model)
      if (corelingPreset) {
        void applyCorelingModelByName(model).then(switchResult => {
          if (switchResult.error) {
            onDone(switchResult.error, { display: 'system' })
          ...
          onDone(
            `Set model to ${chalk.bold(model)}${corelingModelSwitchHint(corelingPreset, switchResult.downloaded)}`,
            { display: 'system' },
          )
        })
        return
...
async function refreshModelsAndSummarize(): Promise<string> {
  ...
  if (discoveryContext?.kind === 'coreling') {
    return 'Coreling v2: Spark · Chat · Pro (local) + free cloud models. Run /model to switch.'
  }
...
export const call: LocalJSXCommandCall = async (onDone, _context, args) => {
  ...
  if (COMMON_HELP_ARGS.includes(trimmedArgs)) {
    onDone(
      'Run /model for Spark · Chat · Pro (local) and free cloud models, /model [name] to set directly.',
      {
        display: 'system',
      },
    )

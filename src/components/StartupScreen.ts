/**
 * Coreling startup screen — filled-block text logo.
...
import { CORELING_V1_STATUS_LINES, CORELING_WOLF_MARK } from '../constants/corelingLogo.js'
...
// ─── Coreling v1 wolf mark ───────────────────────────────────────────────────

// Block-letter CORELING removed — v1 wolf logo is the brand mark.
...
export function printStartupScreen(modelOverride?: string): void {
  ...
  // Coreling v1 wolf mark
  const allLogo = [...CORELING_WOLF_MARK]
  ...
  out.push('')
  for (const line of CORELING_V1_STATUS_LINES) {
    out.push(`  ${DIM}${ansiRgb(...DIMCOL)}${line}${RESET}`)
  }
  ...
  const provC: RGB = p.isLocal ? [80, 250, 123] : ACCENT
  ...
  const sC: RGB = p.isLocal ? [80, 250, 123] : ACCENT
  const sL = p.isLocal ? 'local · private' : 'cloud'

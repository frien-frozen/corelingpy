...
import { BRAND_NAME, BRAND_TAGLINE, WORDMARK_LINES } from '../../constants/brand.js';
...
export function LogoV2() {
  ...
  const borderTitle = ` ${color("text", userTheme)(BRAND_NAME)} ${color("inactive", userTheme)(`v${version}`)} `;
  const compactBorderTitle = color("text", userTheme)(` ${BRAND_NAME} `);
  ...
  if ($[46] !== welcomeMessage_0) {
    t18 = <Box marginTop={1} flexDirection="column" alignItems="center"><Text bold={true} color="brand">{WORDMARK_LINES[0]}</Text><Text dimColor={true}>{WORDMARK_LINES[1]}</Text><Text dimColor={true}>{BRAND_TAGLINE}</Text><Text color="inactive">•</Text><Text bold={true}>{welcomeMessage_0}</Text></Box>;
    $[46] = welcomeMessage_0;
    $[47] = t18;
  } else {
    t18 = $[47];

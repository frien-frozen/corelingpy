...
import { CLI_COMMAND } from './constants/brand.js';
...
async function run(): Promise<CommanderCommand> {
  ...
  program.name('coreling').description(`Coreling - local AI coding agent. Starts an interactive session by default; use -p/--print for non-interactive output`).argument('[prompt]', 'Your prompt', String)
  ...
  .option('--provider-env-file <path>', 'Load provider environment variables from a file before validation (repeatable; existing values win)', (val: string, prev: string[]) => [...prev, val], [] as string[]).action(async (prompt, options) => {
    ...
    if (prompt === 'code') {
      ...
      console.warn(chalk.yellow('Tip: You can launch Coreling with just `coreling`'));
      prompt = undefined;
    }
  ...
  }).version(`${MACRO.DISPLAY_VERSION ?? MACRO.VERSION} (Coreling v2)`, '-v, --version', 'Output the version number');

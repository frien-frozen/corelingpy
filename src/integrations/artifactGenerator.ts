...
function renderIntegrationArtifacts(
  ...
  const orderedProviderPresets = [
    ...(['llama-cpp', 'openrouter'] as const).filter(preset =>
      presetManifest.some(entry => entry.preset === preset),
    ),
    ...presetManifest
    ...
    ...(presetManifest.some(entry => entry.preset === 'gitlawb-opengateway')
      ? (['gitlawb-opengateway'] as const)
      : []),
  ]

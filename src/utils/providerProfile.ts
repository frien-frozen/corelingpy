...
import { getConfigDirName, getProfileFileName } from '../constants/brand.js'
import {
  PLATFORM_OPENROUTER_DEFAULT_MODEL,
  resolveBundledOpenRouterApiKey,
} from '../constants/platformCloud.js'

export const PROFILE_FILE_NAME = getProfileFileName()
...
export function buildOpenRouterProfileEnv(options: {
  ...
  const processEnv = options.processEnv ?? process.env
  const userKey = sanitizeApiKey(options.apiKey ?? processEnv.OPENROUTER_API_KEY)
  const bundledKey = resolveBundledOpenRouterApiKey(processEnv)
  const key = userKey || bundledKey
  if (!key) {
    return null
  }

  const secretSource: SecretValueSource = userKey
  ...
  // Only persist user-supplied keys — bundled platform key stays in the build.
  if (userKey) {
...
export async function buildLaunchEnv(options: {
    ...
    return buildCompatibilityProcessEnv({
      ...
      profileEnv: {
        OPENAI_BASE_URL:
          ...
          getRouteDefaultBaseUrl('llama-cpp') ||
          'http://127.0.0.1:8080/v1',
        OPENAI_MODEL:
          persistedOpenAIModel ||
          getRouteDefaultModel('llama-cpp') ||
          'coreling',
      },
    })
  }
  ...
  if (selectedProfile === 'openrouter') {
    const shellOpenRouterKey = sanitizeApiKey(processEnv.OPENROUTER_API_KEY)
...
export async function buildStartupEnvFromProfile(options?: {
  ...
  if (!persisted) {
    ...
    // No saved profile — default to local llama.cpp (Coreling Engine).
    const env = buildCompatibilityProcessEnv({
      ...
      profileEnv: {
        OPENAI_BASE_URL:
          ...
          'http://127.0.0.1:8080/v1',
        OPENAI_MODEL:
          getRouteDefaultModel('llama-cpp') ?? 'coreling',
      },
    })
    env[DEFAULT_STARTUP_PROVIDER_ENV_VAR] = 'llama-cpp'
    return env
  }
